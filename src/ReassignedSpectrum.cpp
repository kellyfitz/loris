...
//!	Transform lengths are the smallest power of two greater than twice the
//!	window length.
ReassignedSpectrum::ReassignedSpectrum(
...
ReassignedSpectrum::transform(const double *sampsBegin,
                              const double *sampCenter, const double *sampsEnd)
{
    ...
    FourierTransform::iterator it = std::transform(
        sampsBegin, sampsEnd, mCplxWin_W_Wtd.begin() + winBeginOffset,
    ...
    std::rotate(mMagnitudeTransform.begin(),
                mMagnitudeTransform.begin() + rotateBy,
                mMagnitudeTransform.end());
    ...
    it = std::transform(
        sampsBegin, sampsEnd, mCplxWin_Wd_Wt.begin() + winBeginOffset,
        mCorrectionTransform.begin(), std::multiplies<std::complex<double>>());
    //	fill the rest with zeros:
    std::fill(it, mCorrectionTransform.end(), 0.);
    //	rotate to align phase:
    std::rotate(mCorrectionTransform.begin(),
                mCorrectionTransform.begin() + rotateBy,
                mCorrectionTransform.end());
...
ReassignedSpectrum::frequencyCorrection(long idx) const
{
    ...
    double num = X_h.real() * X_Dh.imag() - X_h.imag() * X_Dh.real();

    double magSquared = std::norm(X_h);

    //	need to scale by the oversampling factor
    double oversampling =
        (double)mCorrectionTransform.size() / mCplxWin_W_Wtd.size();
...
ReassignedSpectrum::timeCorrection(long idx) const
{
    ...
    double num = X_h.real() * X_Th.real() + X_h.imag() * X_Th.imag();
    double magSquared = norm(X_h);

    //	No need to scale by the oversampling factor.
    //	No, seems to sound bad, why?
    //	(try alienthreat)
    // double oversampling = (double)mCorrectionTransform.size() /
    // mCplxWin_W_Wtd.size();
...
ReassignedSpectrum::reassignedFrequency(long idx) const
{
    ...
    double dbLeft =
        20. * log10(abs(circEvenPartAt(mMagnitudeTransform, idx - 1)));
    double dbCandidate =
        20. * log10(abs(circEvenPartAt(mMagnitudeTransform, idx)));
    double dbRight =
        20. * log10(abs(circEvenPartAt(mMagnitudeTransform, idx + 1)));

    double peakXOffset =
        0.5 * (dbLeft - dbRight) / (dbLeft - 2.0 * dbCandidate + dbRight);
...
//
template <class T> struct make_complex
{
    std::complex<T>
    operator()(const T &re, const T &im) const
    {
...
void
ReassignedSpectrum::buildReassignmentWindows(
    const std::vector<double> &window,
    const std::vector<double> &windowDerivative)
{
    ...
    std::transform(windowDerivative.begin(), windowDerivative.end(),
                   framp.begin(),
