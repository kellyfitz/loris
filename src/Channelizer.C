    ...
    _ampWeighting(0)
{
    ...
    if (stretchFactor < 0.)
    {
        Throw(InvalidArgument,
              "Channelizer stretch factor must be non-negative.");
    }
}
    ...
    _ampWeighting(0)
{
    ...
    {
        Throw(InvalidArgument,
              "Channelizer reference frequency must be positive.");
    }
    if (stretchFactor < 0.)
    {
        Throw(InvalidArgument,
...
//
double
Channelizer::amplitudeWeighting(void) const
{
...
//
void
Channelizer::setAmplitudeWeighting(double x)
{
    _ampWeighting = x;
}
...
//
double
Channelizer::stretchFactor(void) const
{
...
//
void
Channelizer::setStretchFactor(double stretch)
{
    if (stretch < 0.)
    {
        Throw(InvalidArgument,
...
void
Channelizer::setStretchFactor(double fm, int m, double fn, int n)
{
...
Channelizer::computeStretchFactor(double fm, int m, double fn, int n)
{
    ...
    {
        Throw(InvalidArgument,
              "Channelizer stretched harmonic frequencies must be positive.");
    }
    if (m <= 0 || n <= 0)
    {
        Throw(InvalidArgument,
              "Channelizer stretched harmonic numbers must be positive.");
    }
...
//
double
Channelizer::referenceFrequencyAt(double time) const
{
...
Channelizer::computeFractionalChannelNumber(double time, double frequency) const
{
    ...
    double num = sqrt( (frefsqrd*frefsqrd) + (4*stretch*frefsqrd*fn*fn) ) -
    (frefsqrd); double denom = 2*stretch*frefsqrd; return sqrt( num / denom );
    */

    //  else:
    //  avoid squaring big numbers... two sqrts kind of sucks too.
    const double rB =
        1. / _stretchFactor; // reciprocal of B, the stretch factor
    const double fratio = frequency / refFreq;
    return std::sqrt(std::sqrt((.25 * rB * rB) + (fratio * fratio * rB)) -
                     (.5 * rB));
}
...
Channelizer::channelFrequencyAt(double time, int channel) const
{
    ...
    {
        const double scale =
            std::sqrt(1.0 + (_stretchFactor * channel * channel));
        fn = fn * scale;
    }
    return fn;
}
...
Channelizer::channelize(Partial &partial) const
{
    ...
    // debugger << "channelizing Partial with " << partial.numBreakpoints() << "
    // Breakpoints" << endl;
    ...
    for (bp = partial.begin(); bp != partial.end(); ++bp)
    {
        ...
        if (0 != _ampWeighting)
        {
            ...
            //  emphasized the part of the sound in which the frequency
            //  estimates are least reliable (e.g. a piano tone). The unweighted
            //  average should give more intuitive results in most cases.

            //	use sinusoidal amplitude:
            double a = bp.breakpoint().amplitude() *
                       std::sqrt(1. - bp.breakpoint().bandwidth());
...
//
void
Channelizer::channelize(PartialList &partials, const Envelope &refChanFreq,
                        int refChanLabel)
{
