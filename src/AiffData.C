...
struct extended80
{
    ...
    extended80(double x = 0.)
    {
        ConvertToIeeeExtended(x, this);
    }
    operator double(void) const
    {
        return ConvertFromIeeeExtended(this);
    }
};
...
//
std::istream &
readApplicationSpecifcData(std::istream &s, SosEnvelopesCk &ck,
                           unsigned long chunkSize)
{
    ...
    if (!s)
    {
        Throw(FileIOException, "Failed to read badly-formatted AIFF file (bad "
                               "ApplicationSpecific chunk).");
    }
...
readCommonData(std::istream &s, CommonCk &ck, unsigned long chunkSize)
{
    ...
    if (!s)
    {
        Throw(FileIOException,
              "Failed to read badly-formatted AIFF file (bad Common chunk).");
    }
...
readContainer(std::istream &s, ContainerCk &ck, unsigned long chunkSize)
{
    ...
    if (!s)
    {
        Throw(
            FileIOException,
            "Failed to read badly-formatted AIFF file (bad Container chunk).");
    }
...
readMarkerData(std::istream &s, MarkerCk &ck, unsigned long chunkSize)
{
    ...
    {
        Throw(FileIOException,
              "Failed to read badly-formatted AIFF file (bad Marker chunk).");
    }
...
{
    // debugger << "reading " << bytes.size() << " bytes of sample data" <<
    // endl;
...
readSampleData(std::istream &s, SoundDataCk &ck, unsigned long chunkSize)
{
    ...
    if (!s)
    {
        Throw(
            FileIOException,
            "Failed to read badly-formatted AIFF file (bad Sound Data chunk).");
    }
...
writeCommonData(std::ostream &s, const CommonCk &ck)
{
    ...
    */
    ...
    try
    {
        ...
        BigEndian::write(s, sizeof(extended80), sizeof(char),
                         (char *)&write_rate);
    }
...
{
    //	debugger << "writing " << bytes.size() << " bytes of sample data" <<
    // endl;
...
void
ConvertToIeeeExtended(double num, extended80 *x)
{
    int sign;
    int expon;
    double fMant, fsMant;
...
double
ConvertFromIeeeExtended(const extended80 *x)
{ /* LCN */ /* ? */
    double f;
    int expon;
    Uint_32 hiMant, loMant;
    ...
    hiMant = ((Uint_32)(bytes[2] & 0xFF) << 24) |
             ...
             ((Uint_32)(bytes[4] & 0xFF) << 8) | ((Uint_32)(bytes[5] & 0xFF));
    loMant = ((Uint_32)(bytes[6] & 0xFF) << 24) |
             ((Uint_32)(bytes[7] & 0xFF) << 16) |
             ((Uint_32)(bytes[8] & 0xFF) << 8) | ((Uint_32)(bytes[9] & 0xFF));
