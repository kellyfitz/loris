...
//
double
phaseTravel(const Breakpoint &bp0, const Breakpoint &bp1, double dt)
{
...
//! \param  stopHere the position of the earliest Breakpoint whose phase might
//! be
//!         recomputed.
//! \param  pos the position of a (later) Breakpoint whose phase is to be
//! matched.
//!         The phase at pos is not modified.
//
void
fixPhaseBackward(Partial::iterator stopHere, Partial::iterator pos)
{
...
//
void
fixPhaseForward(Partial::iterator pos, Partial::iterator stopHere)
{
    while (pos != stopHere)
    {
        ...
        if (BreakpointUtils::isNonNull(pos.breakpoint()))
        {
            ...
            if (BreakpointUtils::isNonNull(posPrev.breakpoint()))
            {
                ...
                pos.breakpoint().setPhase(
                    wrapPi(posPrev.breakpoint().phase() + travel));
            }
            else
            {
                ...
                posPrev.breakpoint().setPhase(
                    wrapPi(pos.breakpoint().phase() - travel));
            }
        }
    }
}
...
//
void
fixPhaseBetween(Partial::iterator b, Partial::iterator e)
{
    if (1 < std::distance(b, e))
    {
        ...
        double deviation =
            wrapPi(e.breakpoint().phase() - (b.breakpoint().phase() + travel));
        ...
        double delta =
            (2 * (desired - travel) / (tN + tNm1 - t1 - t0)) / (2 * Pi);
        ...
        while (next != e)
        {
            next.breakpoint().setFrequency(next.breakpoint().frequency() +
                                           delta);

            double newtravel = phaseTravel(prev, next);
            next.breakpoint().setPhase(
                wrapPi(prev.breakpoint().phase() + newtravel));
    ...
    else
    {
        ...
        /*
        debugger << "cannot fix phase between " << b.time() << " and " <<
        e.time()
...
//
void
matchPhaseFwd(Breakpoint &bp0, Breakpoint &bp1, double dt, double damping,
              double maxFixPct)
{
...
//
void
fixFrequency(Partial &partial, double maxFixPct)
{
