...
//
static bool
isPO2(unsigned int N, int *out_expon = 0)
{
    unsigned int M = 1;
    int exp = 0;
    while (M < N)
...
class FTimpl //  FFTW version 3
{
  ...
  public:
        ...
        ftOut(0)
    {
        ...
        {
            Throw(RuntimeError,
                  "FourierTransform could not make a (fftw) plan.");
        }
    }
    ...
    void
    loadInput(const complex<double> *bufPtr)
    {
    ...
    void
    copyOutput(complex<double> *bufPtr) const
    {
        for (FourierTransform::size_type k = 0; k < N; ++k)
        {
...
static void
fftw_die_Loris(const char *s)
{
    using namespace std; // exit might be hidden in there

    notifier << "The FFTW library used by Loris has encountered a fatal error: "
             << s << endl;
    exit(EXIT_FAILURE);
}
...
class FTimpl //  FFTW version 2
{
  ...
  public:
        ...
        ftOut(0)
    {
        ...
        //	create a plan:
        plan = fftw_create_plan_specific(N, FFTW_FORWARD, FFTW_ESTIMATE, ftIn,
                                         1, ftOut, 1);
        ...
        mIsPO2(isPO2(sz))
    {
        ...
        if (mIsPO2)
        {
            ...
            if (0 == mTxInOut || 0 == mTwiddle || 0 == mWorkspace)
            {
                ...
                delete[] mWorkspace;
                Throw(RuntimeError,
                      "FourierTransform: could not initialize tranform");
            }
...
//
static void
slowDFT(double *in, double *out, int N)
{
#if 1
    // slow DFT
    ...
    const std::complex<double> eminj2pioN = std::polar(1.0, -2.0 * Pi / N);
