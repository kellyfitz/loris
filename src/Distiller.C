...
//
static void
fadeInAndOut(Partial &p, double fadeTime)
{
    if (p.first().amplitude() != 0)
    {
        p.insert(p.startTime() - fadeTime,
                 BreakpointUtils::makeNullBefore(p.first(), fadeTime));
    }

    if (p.last().amplitude() != 0)
    {
        p.insert(p.endTime() + fadeTime,
...
//
static void
merge(Partial::const_iterator beg, Partial::const_iterator end,
      Partial &destPartial, double fadeTime, double gapTime = 0.)
{
    ...
    Partial::iterator removeEnd =
        destPartial.findAfter(toMerge.endTime() + gapTime);
    ...
    Partial::iterator removeBegin =
        destPartial.findAfter(toMerge.startTime() - gapTime);
    ...
    double rbt = (removeBegin != destPartial.end()) ? (removeBegin.time())
                                                    : (destPartial.endTime());
    double ret = (removeEnd != destPartial.end()) ? (removeEnd.time())
                                                  : (destPartial.endTime());
    Assert(rbt <= ret);
    ...
    if (removeEnd != destPartial.begin())
    {
        ...
        if (beforeMerge.breakpoint().amplitude() > 0)
        {
            ...
            destPartial.insert(beforeMerge.time() + fadeTime,
                               BreakpointUtils::makeNullAfter(
                                   beforeMerge.breakpoint(), fadeTime));
        }
    }

    //	insert the Breakpoints in the range:
    for (Partial::const_iterator insert = toMerge.begin();
         insert != toMerge.end(); ++insert)
    {
                 ...
                 double gapTime)
{
    ...
    // gap:
    while (cend != pshort.end() && plong.amplitudeAt(cend.time()) == 0 &&
...
//
static bool
distillSorter(const Partial &lhs, const Partial &rhs)
{
...
Distiller::distill_list(PartialList &partials)
{
