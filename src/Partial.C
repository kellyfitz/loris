...
static bool
order_by_time(const Partial_value_type &x, const Partial_value_type &y)
{
...
Partial::insert(double time, const Breakpoint &bp)
{
    ...
    Partial_value_type dummy(time, Breakpoint());
    Partial::container_type::iterator insertHere = std::lower_bound(
        _breakpoints.begin(), _breakpoints.end(), dummy, order_by_time);
    ...
    {
        insertHere =
            _breakpoints.insert(insertHere, Partial_value_type(time, bp));
    }
    return insertHere;
#else
    /*
...
Partial::first(void)
{
    if (size() == 0)
    {
        Throw(InvalidPartial,
              "Tried find first Breakpoint in a Partial with no Breakpoints.");
    }
#if defined(USE_VECTOR)
    //	see note above
...
Partial::last(void)
{
    ...
    {
        Throw(InvalidPartial,
              "Tried find last Breakpoint in a Partial with no Breakpoints.");
    }
#if defined(USE_VECTOR)
    //	see note above
...
Partial::initialPhase(void) const
{
    ...
    {
        Throw(InvalidPartial,
              "Tried find intial phase of a Partial with no Breakpoints.");
    }
    return first().phase();
}
...
Partial::startTime(void) const
{
    ...
    {
        Throw(InvalidPartial,
              "Tried to find start time of a Partial with no Breakpoints.");
    }
    return begin().time();
}
...
Partial::endTime(void) const
{
    ...
    {
        Throw(InvalidPartial,
              "Tried to find end time of a Partial with no Breakpoints.");
    }
    return (--end()).time();
}
...
//!
void
Partial::fadeIn(double fadeTime)
{
...
//!
void
Partial::fadeOut(double fadeTime)
{
...
Partial::parametersAt(double time, double fadeTime) const
{
    ...
    {
        Throw(InvalidPartial,
              "Tried to interpolate a Partial with no Breakpoints.");
    }
