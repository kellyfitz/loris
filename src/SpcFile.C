...
// ---------------------------------------------------------------------------
static void configureEnvelopeDataCk(SoundDataCk &ck,
                                    const SpcFile::partials_type &partials);

static void configureSosMarkerCk(MarkerCk &ck,
                                 const std::vector<Marker> &markers);

static void configureSosEnvelopesCk(SosEnvelopesCk &ck);

static std::ostream &writeSosEnvelopesChunk(std::ostream &s,
                                            const SosEnvelopesCk &ck);

static void configureExportStruct(const SpcFile::partials_type &partials,
                                  double midipitch, double endApproachTime,
                                  int enhanced);
...
//  values are to be modified to make a gradual transition to the static
//  spectrum.
//
...
//
void
SpcFile::write(const std::string &filename, bool enhanced,
...
SpcFile::addPartial(const Partial &p, int label)
{
    ...
    if (label > LargestLabel)
    {
        Throw(InvalidArgument,
              "Spc Partial label is too large, cannot have more than 256.");
    }
...
SpcFile::setMidiNoteNumber(double nn)
{
    ...
    {
        Throw(InvalidArgument,
              "MIDI note number outside of the valid range [1,128]");
    }
    notenum_ = nn;
}
...
struct SpcExportInfo
{
    ...
    double endApproachTime; //  in seconds, this indicates how long before the
                            //  end of the sound the amplitude, frequency, and
                            //  bandwidth values are to be modified to make a
                            //  gradual transition to the spectral content at
                            //  the end, 0.0 indicates no such modifications are
                            //  to be done
    int numPartials;        //  number of partials in spc file
    int fileNumPartials; //  the actual number of partials plus padding to make
                         //  a 2**n value
    ...
    double endTime; //  in seconds, this indicates the time at which to truncate
                    //  the end of the spc file, 0.0 indicates no truncation
    double markerTime; //  in seconds, this indicates time at which a marker is
                       //  inserted in the spc file, 0.0 indicates no marker is
                       //  desired
    double
        sampleRate; //  in hertz, intended sample rate for synthesis of spc file
    double hop;     //  hop size, based on numPartials and sampleRate
    double ampEpsilon; //  small amplitude value (related to lsb value in spc
                       //  file log-amp)
};
...
//  envExp( )
// ---------------------------------------------------------------------------
//  For a range 0x0000 to 0xFFFF, this returns an exponentiated value in the
//  range 0..1. This is the counterpart of SpcFile::envLog().
//
static double
envExp(long intValue)
{
    static double coeff = 65535.0 / log(32768.);
...
//
static double
getPhaseRefTime(int label, const Partial &p, double time)
{
    ...
    while (p.amplitudeAt(time, Fade) < spcEI.ampEpsilon &&
           time < spcEI.endTime + spcEI.hop)
    {
        time += spcEI.hop;
    }
...
//
static void
afbp(const Partial &p, double time, double phaseRefTime, double magMult,
     double freqMult, double &amp, double &freq, double &bw, double &phase)
{

    // Optional endApproachTime processing:
    // Approach amp, freq, and bw values at endTime, and stick at endTime
    // amplitude. We avoid a sudden transition when using stick-at-end-frame
    // sustains. Compute weighting factor between "normal" envelope point and
    // static point.
    if (spcEI.endApproachTime && time > spcEI.endTime - spcEI.endApproachTime)
    {
        ...
        freq = freqMult * (wt * p.frequencyAt(time) +
                           (1.0 - wt) * p.frequencyAt(spcEI.endTime));
        bw = (wt * p.bandwidthAt(time) +
              (1.0 - wt) * p.bandwidthAt(spcEI.endTime));
        phase = p.phaseAt(time);
    }

    // If we are before the phase reference time, or on the final frame,
    // use zero amp and offset phase.
    else if (time < phaseRefTime - spcEI.hop / 2 ||
             time > spcEI.endTime - spcEI.hop / 2)
    {
        amp = 0.;
        freq = freqMult * p.frequencyAt(phaseRefTime);
        bw = 0.;
        phase =
            p.phaseAt(phaseRefTime) - 2. * Pi * (phaseRefTime - time) * freq;
    }
...
//
//  In lval, the log of the sine magnitude occupies the top 8 bits, the log of
//  the frequency occupies the bottom 16 bits.
//
...
//
static void
pack(double amp, double freq, double bw, double phase, Byte *lbytes,
     Byte *rbytes)
{

    // Set phase for one hop earlier, so that Kyma synthesis target phase is
    // correct. Add offset to phase for difference between Kyma and Loris
    // representation.
...
//
static bool
notEmpty(const Partial &p)
{
    return p.size() > 0;
}

static void
packEnvelopes(const SpcFile::partials_type &partials, std::vector<Byte> &bytes)
{
    ...
    // get the reference partial; the lowest-nonzero-labeled partial with any
    // breakpoints
    ...
    for (double tim = spcEI.startTime; tim <= spcEI.endTime; tim += spcEI.hop)
    {
        ...
        for (unsigned int label = 1; label <= spcEI.fileNumPartials; ++label)
        {
...
#endif
            {
                ...
                double magMult = 0.0;
                afbp(refPar, tim, phaseRefTime, magMult, freqMult, amp, freq,
                     bw, phase);
            }
            else
            {
                //  find the reference time for the phase
                double phaseRefTime =
                    getPhaseRefTime(label, partials[label - 1], tim);

                //  find amplitude, frequency, bandwidth, phase value
                afbp(partials[label - 1], tim, phaseRefTime, 1, 1, amp, freq,
                     bw, phase);
            }
...
#define MAX_PSTRING_CHARS 254
        ...
        if (m.markerName.size() % 2 == 0)
            m.markerName += '\0';
        dataSize +=
            sizeof(Uint_16) + sizeof(Uint_32) + (m.markerName.size() + 1);
    }
...
writeSosEnvelopesChunk(std::ostream &s, const SosEnvelopesCk &ck)
{
    ...
    try
    {
        ...
        // The SOSresultion and SOSquasiHarmonic fields are in the phase table
        // memory.
        // BigEndian::write( s, initPhaseLth, sizeof(Int_32), (char
        // *)&ck.initPhase[0] );
...
//
static double
computeHop(int numPartials, double sampleRate)
{
...
//
static double
computeStartTime(const SpcFile::partials_type &pars)
{
    double startTime = 1000.;
    for (SpcFile::partials_type::const_iterator pIter = pars.begin();
         pIter != pars.end(); ++pIter)
        if (pIter->size() > 0 && startTime > pIter->startTime() &&
            pIter->label() > 0)
...
//
static double
computeEndTime(const SpcFile::partials_type &pars)
{
    double endTime = -1000.;
    for (SpcFile::partials_type::const_iterator pIter = pars.begin();
         pIter != pars.end(); ++pIter)
        if (pIter->size() > 0 && endTime < pIter->endTime() &&
            pIter->label() > 0)
...
//
static long
computeNumPartials(const SpcFile::partials_type &pars)
{
                      ...
                      double endApproachTime, int enhanced)
{
    ...
    //  endApproachTime is in seconds; by default it is zero (and has no
    //  effect). a nonzero endApproachTime indicates that the plist does not
    //  include a release, but rather ends in a static spectrum corresponding to
    //  the final breakpoint values of the partials.  the endApproachTime
    ...
    //  in the spc file, 0.0 indicates no marker.  this is not being used
    //  currently.
    spcEI.markerTime = 0.;
    ...
    // spcEI.endTime
    //         << " hop = " << spcEI.hop << " partials = " << spcEI.numPartials
    //         << endl;
}
...
//
static void
processEnhancedPoint(Byte *leftbytes, Byte *rightbytes, const double frameTime,
                     Partial &par)
{
...
//
static void
processSineOnlyPoint(Byte *bytes, const double frameTime, Partial &par)
{
    //  represent bytes as 24 bit integers:
    const int BytesPerSample = 3;
...
SpcFile::readSpcData(const std::string &filename)
{
    ...
    try
    {
        ...
        while (readChunkHeader(s, h))
        {
            ...
            case CommonId:
                ...
                {
                    Throw(FileIOException,
                          "Loris only processes single-channel AIFF samples "
                          "files.");
                }
        ...
        {
            Throw(FileIOException, "Reached end of file before finding both a "
                                   "Common chunk and a Sound Data chunk.");
        }
        if (soseChunk.signature != SosEnvelopesId)
        {
            Throw(FileIOException,
                  "Reached end of file before finding a Spc Envelope Data "
                  "chunk, this must not be a Spc file.");
        }
    }
    catch (Exception &ex)
    {
    ...
    int numPartials =
        enhanced ? soseChunk.validPartials / 2 : soseChunk.validPartials;
    int numFrames = commonChunk.sampleFrames /
                    (fileNumPartials(numPartials) * (enhanced ? 2 : 1));
    double hop =
        soseChunk.resolution * 0.000001; // resolution is in microseconds
    ...
    const int BytesPerSample = 3;
    const int PredictedNumBytes = BytesPerSample * numFrames *
                                  fileNumPartials(numPartials) *
                                  (enhanced ? 2 : 1);
    if (soundDataChunk.sampleBytes.size() != PredictedNumBytes)
    {
        notifier << "Found " << soundDataChunk.sampleBytes.size()
                 << " bytes of " << commonChunk.bitsPerSample
                 << "-bit sample data." << endl;
        ...
        for (int partial = 0; partial < fileNumPartials(numPartials); ++partial)
        {
            if (enhanced)
            {
                ...
                if (partial < partials_.size())
                    processEnhancedPoint(lbytes, rbytes, frame * hop,
                                         partials_[partial]);
            }
            else
            {
                if (partial < partials_.size())
                    processSineOnlyPoint(bytes, frame * hop,
                                         partials_[partial]);
                bytes += BytesPerSample;
            }
        }
