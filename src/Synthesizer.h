...
class Synthesizer
{
  ...
  public:
    ...
    //! Loris Analyzer generally begin and end at non-zero amplitude,
    //! zero-amplitude
    ...
    //!	\param	buffer The vector (of doubles) into which rendered samples
    //!			   should be accumulated.
    //!	\param	fadeTime The Partial fade time in seconds (must be
    //! non-negative).
    //!	\throw	InvalidArgument if the specfied sample rate is non-positive.
    ...
    //	-- implementation --
  private:
    Oscillator m_osc; //  the Synthesizer has-a Oscillator that it uses to
                      //  render all the Partials one by one.

    std::vector<double>
        *m_sampleBuffer; //	samples are computed and stored here, BUT
