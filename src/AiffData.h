...
struct InstrumentCk
{
    ...
    struct Loop
    {
        Int_16 playMode; /* 0 - no loop, 1 - forward looping, 2 - backward
                            looping */
        Uint_16
            beginLoop; /* this is a reference to a markerID, so you always
...
struct SosEnvelopesCk
{
    ...
    #define initPhaseLth ( 4*LargestLabel + 8 )
        Int_32	initPhase[initPhaseLth]; // obsolete initial phase array; is
    VARIABLE LENGTH array
                                // this is big enough for a max of 512 enhanced
    partials plus values below
    //	Int_32	resolution;		// frame duration in microseconds
...
//
std::istream &readApplicationSpecifcData(std::istream &s, SosEnvelopesCk &ck,
...
//
std::istream &readCommonData(std::istream &s, CommonCk &ck,
                             unsigned long chunkSize);
...
// ---------------------------------------------------------------------------
//
std::istream &readContainer(std::istream &s, ContainerCk &ck,
                            unsigned long chunkSize);
...
// ---------------------------------------------------------------------------
//
std::istream &readInstrumentData(std::istream &s, InstrumentCk &ck,
                                 unsigned long chunkSize);
...
// ---------------------------------------------------------------------------
//
std::istream &readMarkerData(std::istream &s, MarkerCk &ck,
                             unsigned long chunkSize);
...
//	positioned, and that the chunk header has already been read.
//
std::istream &readSampleData(std::istream &s, SoundDataCk &ck,
                             unsigned long chunkSize);

// ---------------------------------------------------------------------------
//	configureCommonCk
// ---------------------------------------------------------------------------
void configureCommonCk(CommonCk &ck, unsigned long nFrames, unsigned int nChans,
...
//	configureMarkerCk
// ---------------------------------------------------------------------------
void configureMarkerCk(MarkerCk &ck, const std::vector<Marker> &markers,
                       double srate);
...
// ---------------------------------------------------------------------------
//
void configureSoundDataCk(SoundDataCk &ck, const std::vector<double> &samples,
                          unsigned int bps);
...
//
void convertBytesToSamples(const std::vector<Byte> &bytes,
...
//	contents are overwritten.
//
void convertSamplesToBytes(const std::vector<double> &samples,
