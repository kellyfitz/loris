...
FundamentalFromSamples::buildEnvelope(const double *sampsBeg,
                                      const double *sampsEnd, double sampleRate,
                                      double tbeg, double tend, double interval,
                                      ...
                                      double confidenceThreshold)
{
    ...
    while (time < tend)
    {
        ...
        if (!amplitudes.empty())
        {
            F0Estimate est(amplitudes, frequencies, lowerFreqBound,
                           upperFreqBound, m_precision);
...
FundamentalFromSamples::estimateAt(const double *sampsBeg,
                                   const double *sampsEnd, double sampleRate,
                                   double time, double lowerFreqBound,
                                   double upperFreqBound)
{
    std::vector<double> amplitudes, frequencies;

    collectFreqsAndAmps(sampsBeg, sampsEnd - sampsBeg, sampleRate, frequencies,
                        amplitudes, time);
...
//! analysis.
double
FundamentalFromSamples::windowWidth(void) const
{
...
//! \param x is the new value of this parameter.
void
FundamentalFromSamples::setWindowWidth(double x)
{
...
static bool
sort_peaks_greater_amplitude(const SpectralPeak &lhs, const SpectralPeak &rhs)
{
                                            ...
                                            double time)
{
    ...
    if (m_cacheSampleRate != sampleRate || 0 == m_spectrum.get())
    {
    ...
    const double maxTimeCorrection =
        0.25 * winlen / sampleRate; //  one-quarter the window width
    ...
    {
        m_spectrum->transform(samps + sampsBegin, samps + winMiddle,
                              samps + sampsEnd);
        ...
        if (!peaks.empty())
        {
            ...
            //
            //  (HEY is there any reason to do this, other than to find the
            //  largest?)
            // std::sort( peaks.begin(), peaks.end(),
            // sort_peaks_greater_amplitude );
            Peaks::iterator maxpos = std::max_element(
                peaks.begin(), peaks.end(), sort_peaks_greater_amplitude);

            //  determine the floating amplitude threshold
            const double thresh = std::max(std::pow(10.0, -0.05 * -m_ampFloor),
                                           std::pow(10.0, -0.05 * m_ampRange) *
                                               maxpos->amplitude());

            //  collect amplitudes and frequencies and try to
            //  estimate the fundamental
            for (Peaks::const_iterator spkpos = peaks.begin();
                 spkpos != peaks.end(); ++spkpos)
            {
...
FundamentalFromPartials::FundamentalFromPartials(
    const FundamentalFromPartials &rhs) :
...
LinearEnvelope
FundamentalFromPartials::buildEnvelope(
    PartialList::const_iterator begin_partials,
    PartialList::const_iterator end_partials, double tbeg, double tend,
    double interval, double lowerFreqBound, double upperFreqBound,
    double confidenceThreshold)
{
    //  sanity check
    if (tbeg > tend)
    ...
    while (time < tend)
    {
        collectFreqsAndAmps(begin_partials, end_partials, frequencies,
                            amplitudes, time);
                                    ...
                                    double upperFreqBound)
{
    ...
    collectFreqsAndAmps(begin_partials, end_partials, frequencies, amplitudes,
                        time);
...
void
FundamentalFromPartials::collectFreqsAndAmps(
    PartialList::const_iterator begin_partials,
    PartialList::const_iterator end_partials, std::vector<double> &frequencies,
    std::vector<double> &amplitudes, double time)
{
    amplitudes.clear();
    frequencies.clear();
    ...
    if (begin_partials != end_partials)
    {
        ...
        double max_amp = 0;
        for (PartialList::const_iterator it = begin_partials;
             it != end_partials; ++it)
        {
            //  compute the sinusoidal amplitude (without bandwidth energy)
            double sine_amp =
                std::sqrt(1 - it->bandwidthAt(time)) * it->amplitudeAt(time);
            double freq = it->frequencyAt(time);

            if (sine_amp > thresh && freq < m_freqCeiling)
            {
