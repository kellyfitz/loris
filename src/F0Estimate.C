...
static double secant_method(const vector<double> &amps,
                            const vector<double> &freqs, double f1, double f2,
                            double precision);

static void compute_candidate_freqs(const vector<double> &peak_freqs,
                                    double fmin, double fmax,
                                    vector<double> &eval_freqs);
static void evaluate_Q(const vector<double> &amps, const vector<double> &freqs,
                       const vector<double> &eval_freqs, vector<double> &Q);

static double evaluate_Q(const vector<double> &amps,
                         const vector<double> &freqs, double eval_freq,
                         double norm);

static double evaluate_Qprime(const vector<double> &amps,
                              const vector<double> &freqs, double eval_freq);
...
F0Estimate::F0Estimate(const vector<double> &amps, const vector<double> &freqs,
                       double fmin, double fmax, double resolution) :
    m_frequency(0),
    m_confidence(0)
{
    if (fmin > fmax)
    ...
    if (!eval_freqs.empty())
    {
        ...
        double normalization =
            1.0 /
            std::inner_product(amps.begin(), amps.end(), amps.begin(), 0.0);
        ...
        //     be equal in likelihood to the true F0, but no higher frequency
        //     can be as likely).
        ...
        // -------------------------------------------------------------------------
        // 3)  Refine the best candidate using the secant method for refining
        // the
        //     root of the derivative of the likelihood function in the
        //     neighborhood of the best candidate (because a peak in the
        //     likelihood function is a root of the derivative of that
        //     function).
        ...
        //  the root estimate:
        m_frequency = secant_method(amps, freqs, bestFreq, altFreq, resolution);

        //        notifier << "best candidate is : " << bestFreq
        //                 << " Hz, Q: " << bestQ << endl;
        //        notifier << "secant method found : " << m_frequency
        //                 << " Hz, Q: "
        //                 << evaluate_Q( amps, freqs, m_frequency,
        //                 normalization ) << endl;
        //
        ...
        //
        //  Could also use the bisection method, or the false position method,
        //  which always converge. All that is required is that two points on
        //  the function (the derivative of the likelihood function, in this
        //  case) having opposite signs are used to begin the search. So we need
...
static void
compute_candidate_freqs(const vector<double> &peak_freqs, double fmin,
                        double fmax, vector<double> &eval_freqs)
{
    Assert(fmax > fmin);
...
struct Qterm
{
    ...
    double
    operator()(double amp, double freq) const
    {
...
static double
evaluate_Q(const vector<double> &amps, const vector<double> &freqs,
           double eval_freq, double norm)
{
    double prod = std::inner_product(amps.begin(), amps.end(), freqs.begin(),
                                     0., std::plus<double>(), Qterm(eval_freq));
...
static void
evaluate_Q(const vector<double> &amps, const vector<double> &freqs,
           const vector<double> &eval_freqs, vector<double> &Q, double norm)
{
    Assert(eval_freqs.size() == Q.size());
    Assert(amps.size() == freqs.size());
...
static double
evaluate_Qprime(const vector<double> &amps, const vector<double> &freqs,
                double eval_freq)
{
    double prod =
        std::inner_product(amps.begin(), amps.end(), freqs.begin(), 0.,
                           std::plus<double>(), Qprimeterm(eval_freq));
...
static double
secant_method(const vector<double> &amps, const vector<double> &freqs,
              double f1, double f2, double precision)
{
    ...
    do
    {
        ...
        std::isfinite(deltax) && ++iters < MaxIters);
