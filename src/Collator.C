...
// ---------------------------------------------------------------------------
static bool
ends_earlier(const Partial &lhs, const Partial &rhs)
{
...
//
void
Collator::collateAux(PartialList &unlabeled)
{
    ...
    while (endcollated != unlabeled.end())
    {
        ...
        if (it != endcollated)
        {
            ...
            Breakpoint null2(addme.frequencyAt(nulltime2), 0.,
                             addme.bandwidthAt(nulltime2),
                             addme.phaseAt(nulltime2));
