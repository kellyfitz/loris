 ...
 */
extern "C" void
channelize(PartialList *partials, LinearEnvelope *refFreqEnvelope, int refLabel)
{
    try
    {
        ...
        if (refLabel <= 0)
        {
            Throw(InvalidArgument,
                  "Channelization reference label must be positive.");
        }
    ...
    try
    {
        ...
        if (numSamps != 0)
        {
            env_ptr.reset(new LinearEnvelope(
                FrequencyReference(partials->begin(), partials->end(), minFreq,
                                   maxFreq, numSamps)
                    .envelope()));
        }
        else
                 ...
                 double interval)
{
    try
    {
        ...
        LinearEnvelope *env_ptr = new LinearEnvelope(est.buildEnvelope(
            partials->begin(), partials->end(), span.first, span.second,
            interval, minFreq, maxFreq, Confidence));
        return env_ptr;
    }
    catch (Exception &ex)
    {
 ...
 */
extern "C" void
dilate(PartialList *partials, const double *initial, const double *target,
       int npts)
{
    try
    {
        ...
        Dilator::dilate(partials->begin(), partials->end(), initial,
                        initial + npts, target);
    }
    catch (Exception &ex)
    {
 ...
 */
extern "C" void
exportAiff(const char *path, const double *buffer, unsigned int bufferSize,
 ...
 */
extern "C" void
exportSdif(const char *path, PartialList *partials)
{
    try
    {
        ThrowIfNull((PartialList *)partials);
        ...
        if (partials->size() == 0)
        {
            Throw(Loris::InvalidObject,
                  "No Partials in PartialList to export to sdif file.");
        }
...
/*	Export Partials in a PartialList to a Spc file at the specified file
    ...
    parameter is in seconds. A nonzero endApproachTime indicates that the plist
does not include a release, but rather ends in a static spectrum corresponding
 ...
 */
extern "C" void
exportSpc(const char *path, PartialList *partials, double midiPitch,
          int enhanced, double endApproachTime)
{
    try
    {
        ...
        {
            Throw(InvalidObject,
                  "No Partials in PartialList to export to Spc file.");
        }
        ...
        for (PartialList::iterator iter = partials->begin();
             iter != partials->end(); ++iter)
        {
        ...
        if (countPartials != partials->size())
        {
            notifier << "exporting " << countPartials << " of "
                     << partials->size()
                     << " Partials having labels less than 512." << endl;
        }
        if (countPartials == 0)
        {
            Throw(InvalidObject,
                  "No Partials in PartialList have valid Spc labels (1-511).");
        }
 ...
 */
extern "C" void
harmonify(PartialList *partials, long refLabel, const LinearEnvelope *env,
          double threshold_dB)
{
    try
    {
           ...
           double *samplerate)
{
    ...
    try
    {
        ...
        std::copy(f.samples().begin(), f.samples().begin() + howMany, buffer);
 ...
 */
extern "C" void
importSdif(const char *path, PartialList *partials)
{
    try
    {
        ThrowIfNull((PartialList *)partials);
 ...
 */
extern "C" void
importSpc(const char *path, PartialList *partials)
{
    try
    {
        Loris::notifier << "importing Partials from " << path << Loris::endl;
        Loris::SpcFile imp(path);
        for (Loris::SpcFile::partials_type::const_iterator it =
                 imp.partials().begin();
             it != imp.partials().end(); ++it)
        {
...
extern "C" void
morpher_setAmplitudeShape(double x)
{
    if (x <= 0.)
    {
        std::string s("Loris exception in morpher_setAmplitudeShape(): ");
        s.append("Invalid Argument: the amplitude morph shaping parameter must "
                 "be positive");
        handleException(s.c_str());
    }
    PI_ampMorphShape = x;
 ...
 */
extern "C" void
morph(const PartialList *src0, const PartialList *src1,
      const LinearEnvelope *ffreq, const LinearEnvelope *famp,
      const LinearEnvelope *fbw, PartialList *dst)
{
    try
    {
 ...
 */
extern "C" void
morphWithReference(const PartialList *src0, const PartialList *src1,
                   ...
                   const LinearEnvelope *fbw, PartialList *dst)
{
    try
    {
        ...
        {
            notifier << "using no reference Partial for first morph source"
                     << endl;
        }
        ...
        {
            notifier << "using no reference Partial for second morph source"
                     << endl;
        }
 ...
 */
extern "C" void
resample(PartialList *partials, double interval)
{
    try
    {
        ThrowIfNull((PartialList *)partials);

        notifier << "resampling " << partials->size() << " Partials"
                 << Loris::endl;
 ...
 */
extern "C" void
shapeSpectrum(PartialList *partials, PartialList *surface, double stretchFreq,
              double stretchTime)
{
    try
    {
 ...
 */
extern "C" void
sift(PartialList *partials)
{
    try
    {
        ThrowIfNull((PartialList *)partials);

        notifier << "sifting " << partials->size() << " Partials"
                 << Loris::endl;
 ...
 */
extern "C" unsigned int
synthesize(const PartialList *partials, double *buffer, unsigned int bufferSize,
           double srate)
{
    unsigned int howMany = 0;
    try
    {
        ...
        notifier << "synthesizing " << partials->size() << " Partials at "
                 << srate << " Hz" << endl;
        ...
        std::transform(buffer, buffer + howMany, vec.begin(), buffer,
                       std::plus<double>());
    }
    catch (Exception &ex)
    {
