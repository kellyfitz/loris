...
// ---------------------------------------------------------------------------
static double
accumPeakSquaredAmps(double init, const SpectralPeak &pk)
{
...
template <class Pair>
static double
compare2nd(const Pair &p1, const Pair &p2)
{
...
class FundamentalBuilder : public LinearEnvelopeBuilder
{
  ...
  public:
    FundamentalBuilder(double fmin, double fmax, double threshDb = -60,
                       double threshHz = 8000) :
    ...
    clone(void) const
    {
        return new FundamentalBuilder(*this);
    }
...
//
void
FundamentalBuilder::build(const Peaks &peaks, double frameTime)
{
    amplitudes.clear();
    frequencies.clear();
    for (Peaks::const_iterator spkpos = peaks.begin(); spkpos != peaks.end();
         ++spkpos)
    {
    ...
    if (!amplitudes.empty())
    {
        ...
        if (est.confidence() >= mMinConfidence && est.frequency() > fmin &&
            est.frequency() < fmax)
        {
...
//
void
AmpEnvBuilder::build(const Peaks &peaks, double frameTime)
{
    double x =
        std::accumulate(peaks.begin(), peaks.end(), 0.0, accumPeakSquaredAmps);
...
Analyzer::configure(double resolutionHz, double windowWidthHz)
{
    ...
    //  parameters:
    buildFundamentalEnv(0.99 * resolutionHz, 1.5 * resolutionHz);
...
Analyzer::configure(const Envelope &resolutionEnv, double windowWidthHz)
{
     ...
     */	//	!!!!!!!
    m_f0Builder.reset(new FundamentalBuilder(
        *m_freqResolutionEnv * 0.99, *m_freqResolutionEnv * 1.5, -60., 8000.));
                  ...
                  const Envelope &reference)
{
    ...
    try
    {
        ...
        while (winMiddle < bufEnd)
        {
            ...
            //  sampsBegin is the position of the first sample to be
            //  transformed, sampsEnd is the position after the last sample to
            //  be transformed. (these computations work for odd length windows
            //  only)
            const double *sampsBegin =
                std::max(winMiddle - (winlen / 2), bufBegin);
            const double *sampsEnd =
                std::min(winMiddle + (winlen / 2) + 1, bufEnd);
            ...
            if (m_bwAssocParam > 0)
            {
                bwAssociator->associateBandwidth(peaks.begin(), rejected,
                                                 peaks.end());
            }
...
Analyzer::storeConvergenceBandwidth(double tolerance)
{
    if (1.0 < tolerance)
    {
        ...
        notifier << "Analyzer::storeConvergenceBandwidth, conergence tolerance "
                    "should be positive and less than 1.0, scaling by 1/100"
                 << endl;
        tolerance *= 0.01;
    }

    VERIFY_ARG(storeConvergenceBandwidth, (tolerance > 0) && (tolerance <= 1));
...
//
void
Analyzer::buildFundamentalEnv(double fmin, double fmax, double threshDb,
                              double threshHz)
{
    m_f0Builder.reset(new FundamentalBuilder(fmin, fmax, threshDb, threshHz));
}
...
struct can_mask
{
    ...
    bool
    operator()(const SpectralPeak &v) const
    {
        return (v.frequency() > _fmin) && (v.frequency() < _fmax);
    }
...
Analyzer::thinPeaks(Peaks &peaks, double frameTime)
{
    ...
    while (it != peaks.end())
    {
                                          ...
                                          can_mask(lower, upper)))
        {
            ...
            {
                double alpha =
                    (beginFade - pk.amplitude()) / (beginFade - threshold);
...
//  at 1.0.
void
Analyzer::fixBandwidth(Peaks &peaks)
{
