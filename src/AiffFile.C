...
//
AiffFile::AiffFile(const double *buffer, size_type bufferlength,
                   double samplerate) :
    notenum_(60),
    rate_(samplerate),
    numchans_(1)
...
AiffFile::AiffFile(const std::vector<double> &vec_left,
                   const std::vector<double> &vec_right, double samplerate) :
    notenum_(60),
    rate_(samplerate),
    numchans_(2),
...
AiffFile::write(const std::string &filename, unsigned int bps)
{
    ...
    configureCommonCk(commonChunk, samples_.size() / numchans_, numchans_, bps,
                      rate_);
...
AiffFile::readAiffData(const std::string &filename)
{
    ...
    convertBytesToSamples(soundDataChunk.sampleBytes, samples_,
                          commonChunk.bitsPerSample);
