...
static void importPartials(std::istream &s, PartialList &partials,
                           double bweCutoff);
static void getPartial(std::istream &s, PartialList &partials,
                       double bweCutoff);
...
//
ImportLemur::ImportLemur(const std::string &fname,
                         double bweCutoff /* = 1000 Hz */)
{
    std::fstream fs;
    try
    {
...
importPartials(std::istream &s, PartialList &partials, double bweCutoff)
{
            ...
            else if (h.id == TrackDataID)
            {
                if (!foundParams) //	 I hope this doesn't happen
                {
                    Throw(ImportException,
                          "Mia culpa! I am not smart enough to read the Track "
                          "data before the Analysis Parameters data.");
                }
                //	read Partials:
    ...
    catch (Exception &ex)
    {
        if (s.eof())
        {
            ex.append("Reached end of file before finding both a Tracks chunk "
                      "and a Parameters chunk.");
        }
...
readContainer(std::istream &s)
{
    ...
    {
        ex.append(
            "Failed to read badly-formatted Lemur file (bad Container chunk).");
        throw;
    }
    ...
    {
        Throw(ImportException,
              "File is not formatted correctly for Lemur 5 import.");
    }
}
...
readTracksChunk(std::istream &s)
{
    ...
    {
        ex.append("Failed to read badly-formatted Lemur file (bad Track Data "
                  "chunk).");
        throw;
    }
...
readParamsChunk(std::istream &s)
{
    ...
    {
        ex.append("Failed to read badly-formatted Lemur file (bad Parameters "
                  "chunk).");
        throw;
    }
    ...
    if (ck.formatNumber != FormatNumber)
    {
        Throw(FileIOException,
              "File has wrong Lemur format for Lemur 5 import.");
    }
}
