...
class Analyzer
{
  ...
  public:
    ...
    PartialList analyze(const double *bufBegin, const double *bufEnd,
                        double srate);
    ...
    //!         frequency contour expected of the resulting Partials.
    PartialList analyze(const double *bufBegin, const double *bufEnd,
                        double srate, const Envelope &reference);
    ...
    //! Set the time-varying frequency resolution (minimum instantaneous
    //! frequency difference between Partials) for this Analyzer. (Does not
    //! cause other parameters to be recomputed.)
    //!
    ...
    void storeResidueBandwidth(
        double regionWidth = Default_ResidueBandwidth_RegionWidth);
    ...
    void storeConvergenceBandwidth(
        double tolerancePct =
    ...
    //!         that will constribute to the fundamental frequency estimate
    //!         (very low amplitude peaks tend to have less reliable frequency
    //!         estimates). Default is -60 dB.
    //! \param  threshHz is the upper bound on the frequency of a spectral
    //!         peak that will constribute to the fundamental frequency
    //!         estimate. Default is 8 kHz.
    ...
    //  compatibility.
    void
    buildAmpEnv(bool TF = true)
    {
        TF = TF;
    }
    void
    buildFundamentalEnv(bool TF = true)
    {
        TF = TF;
    }
  ...
  private:
    ...
    double m_cropTime; //!  in seconds, maximum time correction for a spectral
                       //!  component to be considered reliable, and to be
                       //!  eligible for extraction and for Breakpoint formation

    double
        m_bwAssocParam; //!  formerly, width in Hz of overlapping bandwidth
                        //!  association regions, or zero if bandwidth
                        //!  association is disabled, now a catch-all bandwidth
                        //!  association parameter that, if negative, indicates
                        //!  the tolerance (%) level used to construct bandwidth
                        //!  envelopes from the mixed phase derivative indicator

    double m_sidelobeLevel; //!  sidelobe attenutation level for the Kaiser
                            //!  analysis window, in positive dB
