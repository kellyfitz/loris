...
Synthesizer::synthesize(Partial p)
{
    ...
    {
        // debugger << "Synthesizer ignoring a partial that contains no
        // Breakpoints" << endl;
        return;
    }
    ...
    if (p.startTime() < 0)
    {
        Throw(InvalidPartial,
              "Tried to synthesize a Partial having start time less than 0.");
    }
    ...
    */
    ...
    double itime =
        (m_fadeTimeSec < p.startTime()) ? (p.startTime() - m_fadeTimeSec) : 0.;
    index_type currentSamp =
        index_type((itime * m_srateHz) + 0.5); //  cheap rounding
    ...
    m_osc.resetEnvelopes(
        BreakpointUtils::makeNullBefore(p.first(), p.startTime() - itime),
        m_srateHz);
    ...
    {
        index_type tgtSamp =
            index_type((it.time() * m_srateHz) + 0.5); //  cheap rounding
        ...
        if (m_osc.amplitude() == 0.)
        {
            ...
            //
            //  double favg = 0.5 * ( prevFrequency +
            //  it.breakpoint().frequency() ); double dphase = 2 * Pi * favg * (
            //  tgtSamp - currentSamp ) / m_srateHz;
            //
    ...
    m_osc.oscillate(bufferBegin + currentSamp, bufferBegin + endSamp,
                    BreakpointUtils::makeNullAfter(p.last(), m_fadeTimeSec),
                    m_srateHz);
}
...
// static const Synthesizer::EnhancementFlag Default_Enhancement_Flag =
// Synthesizer::BwEnhanced;
...
static Synthesizer::Parameters &
TheSynthesizerDefaultParameters(void)
{
...
//!	\throw	InvalidArgument if any of the parameters is invalid.
void
Synthesizer::SetDefaultParameters(const Synthesizer::Parameters &params)
{
    if (IsValidParameters(params))
    {
...
//!               Synthesizers.
bool
Synthesizer::IsValidParameters(const Parameters &params)
{
    //  check to make sure that the sample rate is valid:
    ...
    {
        Throw(InvalidArgument, "Synthesizer filter zeroeth feedback "
