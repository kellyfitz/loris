...
const double Morpher::DefaultBreakpointGap =
    1E-4; // minimum time (sec) between Breakpoints in
          // morphed Partials
...
Morpher::morphPartials(Partial src, Partial tgt, int assignLabel)
{
    ...
    while (src_iter != src.end() || tgt_iter != tgt.end())
    {
            ...
            (src_iter != src.end() && src_iter.time() < tgt_iter.time()))
        {
            ...
            {
                appendMorphedSrc(src_iter.breakpoint(), tgt, src_iter.time(),
                                 newp);
            }
        ...
        else
        {
            ...
            {
                appendMorphedTgt(tgt_iter.breakpoint(), src, tgt_iter.time(),
                                 newp);
            }
...
Morpher::fixMorphedPhases(Partial &newp) const
{
    if (0 != newp.numBreakpoints())
    {
        ...
        MorphState curstate =
            GetMorphState(_freqFunction->valueAt(bppos.time()));
               ...
               PartialList::const_iterator endTgt)
{
        ...
        if (it->label() != 0)
        {
            ...
            if (match.src.numBreakpoints() != 0)
            {
                Throw(InvalidArgument,
                      "Source Partials must be distilled before morphing.");
            }
            match.src = *it;
        }
    }
    ...
    for (PartialList::const_iterator it = beginTgt; it != endTgt; ++it)
    {
        ...
        if (it->label() != 0)
        {
            ...
            {
                Throw(InvalidArgument,
                      "Target Partials must be distilled before morphing.");
            }
            match.tgt = *it;
        }
    }
                          ...
                          double time) const
{
    ...
    return interpolateParameters(srcBkpt, tgtBkpt, fweight, aweight, bweight);
}
...
//
Breakpoint
Morpher::morphSrcBreakpoint(const Breakpoint &srcBkpt,
                            const Partial &tgtPartial, double time) const
{
    if (0 == tgtPartial.numBreakpoints())
    {
        Throw(InvalidArgument,
              "morphSrcBreakpoint cannot morph with empty Partial");
    }
...
//
Breakpoint
Morpher::morphTgtBreakpoint(const Breakpoint &tgtBkpt,
                            const Partial &srcPartial, double time) const
{
    if (0 == srcPartial.numBreakpoints())
    {
        Throw(InvalidArgument,
              "morphTgtBreakpoint cannot morph with empty Partial");
    }
...
Morpher::fadeSrcBreakpoint(Breakpoint bp, double time) const
{
    ...
    bp.setAmplitude(interpolateAmplitude(bp.amplitude(), 0, alpha));
    return bp;
}
...
{
    double alpha = _ampFunction->valueAt(time);
    bp.setAmplitude(interpolateAmplitude(0, bp.amplitude(), alpha));
    return bp;
}
...
Morpher::setSourceReferencePartial(const Partial &p)
{
    if (p.label() == 0)
    {
        Throw(InvalidArgument, "the morphing source reference Partial must be "
                                   ...
                                   Partial::label_type refLabel)
{
    if (refLabel != 0)
    {
        ...
        {
            Throw(InvalidArgument,
                  "no Partial has the specified reference label");
        }
        _srcRefPartial = *pos;
    }
    else
...
Morpher::setTargetReferencePartial(const Partial &p)
{
    if (p.label() == 0)
    {
        Throw(InvalidArgument, "the morphing target reference Partial must be "
                               "labeled with its harmonic number");
    }
    _tgtRefPartial = p;
}
...
double
Morpher::amplitudeShape(void) const
{
...
void
Morpher::setAmplitudeShape(double x)
{
    if (x <= 0.)
    {
        Throw(InvalidArgument,
...
double
Morpher::minBreakpointGap(void) const
{
...
//
void
Morpher::setMinBreakpointGap(double x)
{
    if (x <= 0.)
    {
...
//
void
Morpher::morph_aux(PartialCorrespondence &correspondence)
{
    ...
    for (it = correspondence.begin(); it != correspondence.end(); ++it)
    {
        ...
        */
        ...
        {
            if (src.first().amplitude() != 0.0 &&
                src.startTime() > _minBreakpointGapSec)
            {
        ...
        {
            if (tgt.first().amplitude() != 0.0 &&
                tgt.startTime() > _minBreakpointGapSec)
            {
...
//
static void
adjustFrequency(Breakpoint &bp, const Partial &ref,
                Partial::label_type harmonicNum, double thresholdDb,
                double time)
{
    if (ref.numBreakpoints() != 0)
    {
        ...
        const double BeginFade =
            std::pow(10., 0.05 * (thresholdDb + FadeRangeDB));
...
//
static inline bool
partial_is_nonnull(const Partial &p)
{
...
//
inline double
Morpher::interpolateFrequency(double srcFreq, double tgtFreq,
                              double alpha) const
{
...
//
inline Breakpoint
Morpher::interpolateParameters(const Breakpoint &srcBkpt,
                               const Breakpoint &tgtBkpt, double fweight,
                               double aweight, double bweight) const
{
    Breakpoint morphed;

    // interpolate frequencies:
    morphed.setFrequency(interpolateFrequency(srcBkpt.frequency(),
                                              tgtBkpt.frequency(), fweight));

    // interpolate LOG amplitudes:
    morphed.setAmplitude(interpolateAmplitude(srcBkpt.amplitude(),
                                              tgtBkpt.amplitude(), aweight));

    // interpolate bandwidth:
    morphed.setBandwidth(interpolateBandwidth(srcBkpt.bandwidth(),
                                              tgtBkpt.bandwidth(), bweight));
                          ...
                          double time, Partial &newp)
{
    ...
    //  explicitly.
    bool needNull =
        (newp.numBreakpoints() != 0) && (newp.last().amplitude() != 0) &&
        (srcBkpt.amplitude() == 0) && (tgtPartial.numBreakpoints() != 0) &&
    ...
    if (fweight < MaxMorphParam || aweight < MaxMorphParam ||
        bweight < MaxMorphParam || needNull)
    {

        // adjust source Breakpoint frequencies according to the reference
        // Partial (if a reference has been specified):
        adjustFrequency(srcBkpt, _srcRefPartial, newp.label(),
                        _freqFixThresholdDb, time);
        ...
        else
        {
            ...
            // Partial (if a reference has been specified):
            adjustFrequency(tgtBkpt, _tgtRefPartial, newp.label(),
                            _freqFixThresholdDb, time);

            // compute interpolated Breakpoint parameters:
            Breakpoint morphed = interpolateParameters(
                srcBkpt, tgtBkpt, fweight, aweight, bweight);
                          ...
                          double time, Partial &newp)
{
    ...
    bool needNull =
        (newp.numBreakpoints() != 0) && (newp.last().amplitude() != 0) &&
        (tgtBkpt.amplitude() == 0) && (srcPartial.numBreakpoints() != 0) &&
    ...
    if (fweight > MinMorphParam || aweight > MinMorphParam ||
        bweight > MinMorphParam || needNull)
    {
