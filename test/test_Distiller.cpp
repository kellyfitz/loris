...
//
static void
test_distill_nonoverlapping(void)
{
    std::cout << "\t--- testing distill on "
...
//
static void
test_distill_overlapping2(void)
{
    ...
    for (Partial::iterator distit = l.front().begin();
         distit != l.front().end(); ++distit)
    {
        cout << distit.time() << " " << distit.breakpoint().frequency() << endl;
    }
    ...
    double t = 0 + fade;
    compare.insert(
        t, Breakpoint(p1.frequencyAt(t), 0, p1.bandwidthAt(t), p1.phaseAt(t)));
...
//
static void
test_distill_overlapping3(void)
{
    ...
    t = 0.32 - fade;
    compare.insert(t, Breakpoint(p3.frequencyAt(t), 0, 0, p3.phaseAt(t)));
...
test_collate(void)
{
    ...
    t = 0.33 - fade;
    compare1.insert(
        t, Breakpoint(p3.frequencyAt(t), 0, p3.bandwidthAt(t), p3.phaseAt(t)));
