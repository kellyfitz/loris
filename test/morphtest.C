...
main(void)
{
    ...
    try
    {
        ...
        FrequencyReference clarRef(clar.begin(), clar.end(), 415 * .8,
                                   415 * 1.2, 50);
        ...
        {
            throw std::runtime_error("SDIF import yields a different number of "
                                     "partials than were exported!");
        }
        clar = ip.partials();

        // shift pitch of clarinet partials
        cout << "shifting pitch of " << clar.size() << " Partials by 600 cents"
             << endl;
        ...
        cout << "Analyzer will build a fundamental frequency estimate for the "
                "flute"
             << endl;
        ...
        cout << "flute fundamental envelope starts at time " << est_time
             << endl;
        while (est_time < 2)
        {
            cout << "flute fundamental estimate at time " << est_time << " is "
                 << fund.valueAt(est_time) << endl;
            est_time += 0.35;
        }

        //  generate a sinusoid that tracks the fundamental
        //	and amplitude envelopes obtained during analysis
        cout << "synthesizing sinusoid from flute amp and fundamental estimates"
             << endl;
        Partial f1;
        ...
        cout << "dilating sounds to match (" << tgt_times[0] << ", "
             << tgt_times[1] << ")" << endl;
        cout << "flute times: (" << flute_times[0] << ", " << flute_times[1]
             << ")" << endl;
        Dilator::dilate(flut.begin(), flut.end(), flute_times, flute_times + 2,
                        tgt_times);

        cout << "clarinet times: (" << clar_times[0] << ", " << clar_times[1]
             << ")" << endl;
        Dilator::dilate(clar.begin(), clar.end(), clar_times, clar_times + 2,
                        tgt_times);
        ...
        cout << "synthesizing " << m.partials().size() << " morphed partials"
             << endl;
        AiffFile morphout(m.partials().begin(), m.partials().end(),
                          f.sampleRate());
