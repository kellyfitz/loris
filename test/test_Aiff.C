...
//
static std::string
make_twoMarkers(const std::string &in_fname,
...
main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        PartialList::iterator fundIt = std::find_if(
            clar.begin(), clar.end(), PartialUtils::isLabelEqual(1));
        if (fundIt == clar.end())
        {
            cout << "FAILED: no partial labeled 1 (fundamental) was found."
                 << endl;
            return 1;
        }
        Partial &fund = *fundIt;
        cout << "fundamental appears to be about " << fund.frequencyAt(1)
             << " Hz." << endl;

        PartialList::iterator secondIt = std::find_if(
            clar.begin(), clar.end(), PartialUtils::isLabelEqual(2));
        if (secondIt == clar.end())
        {
            cout << "FAILED: no partial labeled 2 (second harmonic) was found."
                 << endl;
            return 1;
        }
        Partial &second = *secondIt;
        cout << "second harmonic appears to be about " << second.frequencyAt(1)
             << " Hz." << endl;
        ...
        std::sort( spc.markers().begin(), spc.markers().end(),
        Marker::sortByTime() ); spc.write( "spcSines.ctest.spc", false );
