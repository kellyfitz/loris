...
static void computePhaseFwd(Partial::iterator b,
                            Partial::iterator e); // at bottom
...
makep1(void)
{
    ...
    {
        Breakpoint bp(f0 + ((t - t0) * fslope), a0 + ((t - t0) * aslope),
                      b0 + ((t - t0) * bslope),
...
main()
{
    ...
    std::cout << "Relies on Partial, Breakpoint, and LinearEnvelope." << endl
              << endl;
    ...
    try
    {
        ...
        //  frequency envelope: (0,0), (.2,0), (.4, .5), (.6, .5), (.8, 1),
        //  (1,1)
...
#define SAME_ENV_VALUES(x, y) TEST(float_equal((x), (y)))
        SAME_ENV_VALUES(testM.frequencyFunction().valueAt(.3),
                        fenv.valueAt(.3));
        SAME_ENV_VALUES(testM.frequencyFunction().valueAt(.6),
                        fenv.valueAt(.6));
        SAME_ENV_VALUES(testM.frequencyFunction().valueAt(.9),
                        fenv.valueAt(.9));

        SAME_ENV_VALUES(testM.amplitudeFunction().valueAt(.3),
                        aenv.valueAt(.3));
        SAME_ENV_VALUES(testM.amplitudeFunction().valueAt(.6),
                        aenv.valueAt(.6));
        SAME_ENV_VALUES(testM.amplitudeFunction().valueAt(.9),
                        aenv.valueAt(.9));

        SAME_ENV_VALUES(testM.bandwidthFunction().valueAt(.3),
                        bwenv.valueAt(.3));
        SAME_ENV_VALUES(testM.bandwidthFunction().valueAt(.6),
                        bwenv.valueAt(.6));
        SAME_ENV_VALUES(testM.bandwidthFunction().valueAt(.9),
                        bwenv.valueAt(.9));
...
#define LABEL 2
        ...
        // in the sources:
        TEST(pmorphed.numBreakpoints() ==
             p1.numBreakpoints() + p2.numBreakpoints());
...
#define SAME_PARAM_VALUES(x, y) TEST(float_equal((x), (y)))
        ...
        SAME_PARAM_VALUES(pmorphed.frequencyAt(p2.endTime()),
                          p2.frequencyAt(p2.endTime()));
        ...
        SAME_PARAM_VALUES(pmorphed.amplitudeAt(p1.startTime()),
                          p1.amplitudeAt(p1.startTime()));
        SAME_PARAM_VALUES(pmorphed.amplitudeAt(p2.endTime()),
                          p2.amplitudeAt(p2.endTime()));
        ...
        for (it = p1.begin(); it != p1.end(); ++it)
        {
            ...
            double t = it.time();
            Breakpoint bp2(p2.frequencyAt(t), p2.amplitudeAt(t),
        ...
        for (it = p2.begin(); it != p2.end(); ++it)
        {
            ...
            double t = it.time();
            Breakpoint bp1(p1.frequencyAt(t), p1.amplitudeAt(t),
        ...
        for (Partial::iterator dummy_iter = to_dummy_by_hand.begin();
             dummy_iter != to_dummy_by_hand.end(); ++dummy_iter)
        {
        ...
        Partial to_dummy =
            testM.morphPartial(p1, Partial(), to_dummy_by_hand.label());
        ...
        SAME_PARAM_VALUES(to_dummy.frequencyAt(0),
                          to_dummy_by_hand.frequencyAt(0));
        SAME_PARAM_VALUES(to_dummy.amplitudeAt(0),
                          to_dummy_by_hand.amplitudeAt(0));
        SAME_PARAM_VALUES(to_dummy.bandwidthAt(0),
                          to_dummy_by_hand.bandwidthAt(0));
        SAME_PARAM_VALUES(m2pi(to_dummy.phaseAt(0)),
                          m2pi(to_dummy_by_hand.phaseAt(0)));

        SAME_PARAM_VALUES(to_dummy.frequencyAt(0.1),
                          to_dummy_by_hand.frequencyAt(0.1));
        SAME_PARAM_VALUES(to_dummy.amplitudeAt(0.1),
                          to_dummy_by_hand.amplitudeAt(0.1));
        SAME_PARAM_VALUES(to_dummy.bandwidthAt(0.1),
                          to_dummy_by_hand.bandwidthAt(0.1));
        SAME_PARAM_VALUES(m2pi(to_dummy.phaseAt(0.1)),
                          m2pi(to_dummy_by_hand.phaseAt(0.1)));

        SAME_PARAM_VALUES(to_dummy.frequencyAt(0.3),
                          to_dummy_by_hand.frequencyAt(0.3));
        SAME_PARAM_VALUES(to_dummy.amplitudeAt(0.3),
                          to_dummy_by_hand.amplitudeAt(0.3));
        SAME_PARAM_VALUES(to_dummy.bandwidthAt(0.3),
                          to_dummy_by_hand.bandwidthAt(0.3));
        SAME_PARAM_VALUES(m2pi(to_dummy.phaseAt(0.3)),
                          m2pi(to_dummy_by_hand.phaseAt(0.3)));

        SAME_PARAM_VALUES(to_dummy.frequencyAt(0.6),
                          to_dummy_by_hand.frequencyAt(0.6));
        SAME_PARAM_VALUES(to_dummy.amplitudeAt(0.6),
                          to_dummy_by_hand.amplitudeAt(0.6));
        SAME_PARAM_VALUES(to_dummy.bandwidthAt(0.6),
                          to_dummy_by_hand.bandwidthAt(0.6));
        SAME_PARAM_VALUES(m2pi(to_dummy.phaseAt(0.6)),
                          m2pi(to_dummy_by_hand.phaseAt(0.6)));

        SAME_PARAM_VALUES(to_dummy.frequencyAt(0.85),
                          to_dummy_by_hand.frequencyAt(0.85));
        SAME_PARAM_VALUES(to_dummy.amplitudeAt(0.85),
                          to_dummy_by_hand.amplitudeAt(0.85));
        SAME_PARAM_VALUES(to_dummy.bandwidthAt(0.85),
                          to_dummy_by_hand.bandwidthAt(0.85));
        SAME_PARAM_VALUES(m2pi(to_dummy.phaseAt(0.85)),
                          m2pi(to_dummy_by_hand.phaseAt(0.85)));

        SAME_PARAM_VALUES(to_dummy.frequencyAt(1),
                          to_dummy_by_hand.frequencyAt(1));
        SAME_PARAM_VALUES(to_dummy.amplitudeAt(1),
                          to_dummy_by_hand.amplitudeAt(1));
        SAME_PARAM_VALUES(to_dummy.bandwidthAt(1),
                          to_dummy_by_hand.bandwidthAt(1));
        SAME_PARAM_VALUES(m2pi(to_dummy.phaseAt(1)),
                          m2pi(to_dummy_by_hand.phaseAt(1)));
        ...
        for (Partial::iterator dummy_iter = from_dummy_by_hand.begin();
             dummy_iter != from_dummy_by_hand.end(); ++dummy_iter)
        {
            double t = dummy_iter.time();
            double a = dummy_iter.breakpoint().amplitude();
        ...
        Partial from_dummy =
            testM.morphPartial(Partial(), p1, from_dummy_by_hand.label());

        //  check:
        TEST(from_dummy.label() == from_dummy_by_hand.label());
        TEST(from_dummy.numBreakpoints() ==
             from_dummy_by_hand.numBreakpoints());

        SAME_PARAM_VALUES(from_dummy.startTime(),
                          from_dummy_by_hand.startTime());
        ...
        SAME_PARAM_VALUES(from_dummy.frequencyAt(0),
                          from_dummy_by_hand.frequencyAt(0));
        SAME_PARAM_VALUES(from_dummy.amplitudeAt(0),
                          from_dummy_by_hand.amplitudeAt(0));
        SAME_PARAM_VALUES(from_dummy.bandwidthAt(0),
                          from_dummy_by_hand.bandwidthAt(0));
        SAME_PARAM_VALUES(m2pi(from_dummy.phaseAt(0)),
                          m2pi(from_dummy_by_hand.phaseAt(0)));

        SAME_PARAM_VALUES(from_dummy.frequencyAt(0.1),
                          from_dummy_by_hand.frequencyAt(0.1));
        SAME_PARAM_VALUES(from_dummy.amplitudeAt(0.1),
                          from_dummy_by_hand.amplitudeAt(0.1));
        SAME_PARAM_VALUES(from_dummy.bandwidthAt(0.1),
                          from_dummy_by_hand.bandwidthAt(0.1));
        SAME_PARAM_VALUES(m2pi(from_dummy.phaseAt(0.1)),
                          m2pi(from_dummy_by_hand.phaseAt(0.1)));

        SAME_PARAM_VALUES(from_dummy.frequencyAt(0.3),
                          from_dummy_by_hand.frequencyAt(0.3));
        SAME_PARAM_VALUES(from_dummy.amplitudeAt(0.3),
                          from_dummy_by_hand.amplitudeAt(0.3));
        SAME_PARAM_VALUES(from_dummy.bandwidthAt(0.3),
                          from_dummy_by_hand.bandwidthAt(0.3));
        SAME_PARAM_VALUES(m2pi(from_dummy.phaseAt(0.3)),
                          m2pi(from_dummy_by_hand.phaseAt(0.3)));

        SAME_PARAM_VALUES(from_dummy.frequencyAt(0.6),
                          from_dummy_by_hand.frequencyAt(0.6));
        SAME_PARAM_VALUES(from_dummy.amplitudeAt(0.6),
                          from_dummy_by_hand.amplitudeAt(0.6));
        SAME_PARAM_VALUES(from_dummy.bandwidthAt(0.6),
                          from_dummy_by_hand.bandwidthAt(0.6));
        SAME_PARAM_VALUES(m2pi(from_dummy.phaseAt(0.6)),
                          m2pi(from_dummy_by_hand.phaseAt(0.6)));

        SAME_PARAM_VALUES(from_dummy.frequencyAt(0.85),
                          from_dummy_by_hand.frequencyAt(0.85));
        SAME_PARAM_VALUES(from_dummy.amplitudeAt(0.85),
                          from_dummy_by_hand.amplitudeAt(0.85));
        SAME_PARAM_VALUES(from_dummy.bandwidthAt(0.85),
                          from_dummy_by_hand.bandwidthAt(0.85));
        SAME_PARAM_VALUES(m2pi(from_dummy.phaseAt(0.85)),
                          m2pi(from_dummy_by_hand.phaseAt(0.85)));

        SAME_PARAM_VALUES(from_dummy.frequencyAt(1),
                          from_dummy_by_hand.frequencyAt(1));
        SAME_PARAM_VALUES(from_dummy.amplitudeAt(1),
                          from_dummy_by_hand.amplitudeAt(1));
        SAME_PARAM_VALUES(from_dummy.bandwidthAt(1),
                          from_dummy_by_hand.bandwidthAt(1));
        SAME_PARAM_VALUES(m2pi(from_dummy.phaseAt(1)),
                          m2pi(from_dummy_by_hand.phaseAt(1)));
    }
    catch (Exception &ex)
    {
...
//
static double
phaseTravel(Partial::const_iterator bp0, Partial::const_iterator bp1)
{
...
//
static void
computePhaseFwd(Partial::iterator b, Partial::iterator e)
{
