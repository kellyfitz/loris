...
static void
float_rel_equal(double x, double y, double pct)
{
#ifdef VERBOSE
    cout << "\t" << x << " == " << y << " ?" << endl;
#endif
...
one_partial(void)
{
    ...
    cout << "Using analysis window width " << anal.windowWidth() << " Hz"
         << endl;
    ...
    while (t <= p1.endTime())
    {
        cout << t << "\t" << p1.amplitudeAt(t, 0.01) << "  "
             << a1.amplitudeAt(t, 0.01) << endl;
    ...
    while (t <= p1.endTime())
    {
        ...
        {
            cout << t << "\t" << p1.frequencyAt(t) << "  " << a1.frequencyAt(t)
                 << endl;
    ...
    while (t <= p1.endTime())
    {
        if (p1.amplitudeAt(t) > 0)
        {
            cout << t << "\t" << mpi(p1.phaseAt(t)) / pi << " "
                 << mpi(a1.phaseAt(t)) / pi << endl;
            float_abs_equal(mpi(p1.phaseAt(t)) / pi, mpi(a1.phaseAt(t)) / pi,
                            0.01 * pi);
        }
        t = t + dt;
    }
...
two_partials(void)
{
    ...
    */
    ...
    cout << "START TIMES (p1 a1 p2 a2) (testing within " << hop * 1000 << "ms)"
         << endl;
    cout << p1.startTime() << "  " << a1.startTime() << "\t" << p2.startTime()
         << "  " << a2.startTime() << endl;
    float_abs_equal(p1.startTime(), a1.startTime(), hop);
    float_abs_equal(p2.startTime(), a2.startTime(), hop);

    cout << "END TIMES (p1 a1 p2 a2) (testing within " << hop * 1000 << "ms)"
         << endl;
    cout << p1.endTime() << "  " << a1.endTime() << "\t" << p2.endTime() << "  "
         << a2.endTime() << endl;
    ...
    {
        cout << t << "\t" << p1.amplitudeAt(t, 0.01) << "  "
             << a1.amplitudeAt(t, 0.01) << "\t" << p2.amplitudeAt(t, 0.01)
             << "  " << a2.amplitudeAt(t, 0.01) << endl;
        float_rel_equal(p1.amplitudeAt(t, 0.01), a1.amplitudeAt(t, 0.01), 0.02);
    ...
    while (t < tmax - dt)
    {
        ...
        if (p2.amplitudeAt(t) > 0)
        {
            ...
            float_abs_equal(mpi(p2.phaseAt(t)) / pi, mpi(a2.phaseAt(t)) / pi,
                            0.01 * pi);
        }
        cout << endl;
...
main(void)
{
    ...
    std::cout << "and compares the analyzed Partials to the originals." << endl
              << endl;
