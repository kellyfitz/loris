...
renderStandard(PartialList &partials)
{
    vector<double> buf;
    Synthesizer synth(Rate, buf);
    for (PartialList::iterator it = partials.begin(); it != partials.end();
         ++it)
    {
        synth.synthesize(*it);
    }
...
static vector<double>
renderFast(PartialList &partials)
{
    ...
    BlockSynthBwe synth(Fastsynth_BlockSize_samples, Rate,
                        reader.numPartials());
...
static long
bestLag(const vector<double> &a, const vector<double> &b, long maxlag,
        int *corrSign = 0)
{
    long best = 0;
    double bestcorr = 0;
...
//  over a +/- 10 ms window
static double
measurePhase(const vector<double> &x, double t, double freq)
{
...
static DiffStats
sampleDiff(const vector<double> &a, const vector<double> &b, long lag,
           size_t skip)
{
    ...
    s.maxAbs = maxAbs;
    s.rmsErrDb =
        (n && refsq > 0 && sumsq > 0) ? 10 * std::log10(sumsq / refsq) : -999;
    return s;
}

//  short-time RMS envelope (winSamps window, hop = winSamps)
static vector<double>
rmsEnvelope(const vector<double> &x, size_t winSamps)
{
    vector<double> env;
...
static Partial
makeSteady(double freq, double amp, double dur)
{
    Partial p;
    p.insert(0, Breakpoint(freq, amp, 0, 0));
    p.insert(dur,
             Breakpoint(freq, amp, 0, std::fmod(TwoPi * freq * dur, TwoPi)));
    return p;
}

static Partial
makeGlide(double f0, double f1, double amp, double dur)
{
    Partial p;
...
static Partial
makeRamp(double freq, double amp1, double dur)
{
    Partial p;
    p.insert(0, Breakpoint(freq, 0, 0, 0));
    p.insert(dur,
             Breakpoint(freq, amp1, 0, std::fmod(TwoPi * freq * dur, TwoPi)));
    return p;
}
...
static DiffStats
compareCase(const std::string &name, const Partial &p)
{
    PartialList plist;
    plist.push_back(p);
    ...
    DiffStats s =
        sampleDiff(std_out, fast_out, lag, 3 * Fastsynth_BlockSize_samples);

    int sign = 1;
    long xlag =
        bestLag(std_out, fast_out, 2 * Fastsynth_BlockSize_samples, &sign);

    cout << "  " << std::left << std::setw(26) << name << "max|diff| "
         << std::setw(12) << s.maxAbs << "rms err " << std::setprecision(4)
         << std::setw(8) << s.rmsErrDb << " dB   (xcorr lag " << xlag
         << (sign < 0 ? " NEG)" : ")") << endl;
    return s;
}
...
static double
phaseProbe(const std::string &name, const Partial &p, double f0,
           double chirpRate, long lagComp)
{
    PartialList plist;
    plist.push_back(p);
    vector<double> std_out = renderStandard(plist);
    ...
    for (double t = 0.1; t < 0.45; t += 0.1)
    {
        ...
        cout << "    " << std::fixed << std::setprecision(1) << t << "     "
             << std::setw(6) << instf << "      " << std::setprecision(3)
             << std::setw(8) << wrapPhase(stdPh - analytic) << "      "
             << std::setw(8) << fastErr << std::defaultfloat << endl;
    }
    return worst;
}
...
main(void)
{
    ...
    DiffStats steady = compareCase("steady 397 Hz", makeSteady(397, 0.5, 0.5));
    DiffStats glide =
        compareCase("glide 397->595 Hz", makeGlide(397, 595.5, 0.5, 0.5));
    DiffStats ramp = compareCase("amp ramp 0->0.5", makeRamp(397, 0.5, 0.5));
    ...
    double worstPh = phaseProbe("steady 397 Hz", makeSteady(397, 0.5, 0.5), 397,
                                0, Fastsynth_BlockSize_samples);
    worstPh = std::max(worstPh, phaseProbe("glide 397->595 Hz",
                                           makeGlide(397, 595.5, 0.5, 0.5), 397,
                                           (595.5 - 397) / 0.5,
                                           Fastsynth_BlockSize_samples));
    if (worstPh > 0.1)
    {
    ...
    {
        if (env_std[i] > peak * 0.01 &&
            env_fast[i] > 0) //  within 40 dB of peak
        {
    ...
    cout << "clarinet envelope deviation (10 ms RMS, active region): mean "
         << meanDevDb << " dB, max " << maxDevDb << " dB over " << n
         << " windows (alignment lag " << lag << ")" << endl;
